...
- **下载到服务器（片库）**：播放时在字幕菜单上方的工具条点「下载到服务器」，或在影片详情页点顶栏的下载图标、在右侧面板里挑一个种子片源，服务器就在后台把整部片下完。**下载期间照常能看**：下载中的片子在片源列表里显示为「下载中 42% · 可边下边播」。下完之后，这部片的片源列表**第一条**就是「已下载 · 服务器本地」，从磁盘直接读，拖动秒开，做种的人都走了也能看；首页还多一个「已下载」目录。下载时会在片源自带的 tracker 之外补一批公共 tracker。任务状态落盘，服务重启后从断点继续；连接中断或长时间没数据会自动重试。
  - **两种下载器**：装了 `aria2c` 就用它下（先取种子信息、只选要的那个文件，`.aria2` 控制文件负责续传）；aria2 没装、连续三次失败或十分钟没有数据，就清掉它的目录、改由本机的 streaming server 下（把这个文件经它的 HTTP 接口从头读到尾，它会按顺序取齐所有分片并照常校验）。实测同一个「做种的人多但难连」的种子，75 秒里 aria2 连上 75–80 个 peer、速度爬到 3.7 MiB/s，streaming server 只连上 6 个、0.6 MB/s；好连的种子两者都能跑满链路。代价是用 aria2 时下载和观看不共用分片，边下边看时你看的那一段会由 streaming server 另下一份；用 streaming server 下载时则是共用的。`LIBRARY_DOWNLOADER=engine` 可以强制只用后者。另外，点下载时如果 streaming server 的缓存里这个文件已经有九成以上（比如刚看完这部片），就直接从它那里读出来，2 GB 只要几秒，不再让 aria2 从种子网络重新下一遍。面板里能看每个任务的进度、速度、连接数和剩余时间，看片库占用和磁盘剩余，取消下载或删除（删除要点两次确认）。片库有配额（默认 50 GB）并保证磁盘留有余量（默认 10 GB），超了就拒绝新任务。浏览器能直接播的文件（MP4 + H.264 + AAC）由网页服务器按 Range 直接发；其余（MKV、HEVC、AC3 等）仍经 streaming server 转封装，只是数据源换成了本地文件。从片库播放时字幕对齐读的也是本地文件。
...
- **Chrome 里 x265 片源画面每十来秒跳一下**：上面那个补丁只是让播放器不再打转，直通的 HEVC 分片交界处缺的那几帧补不回来，所以 Chrome 里画面仍会周期性地跳，而 Safari（只上报 H.264，拿到的是转码后均匀的 4 秒分片）很顺。播放器只在加载时问一次浏览器「能不能播 HEVC」，所以 `deploy.sh` 在播放器脚本之前插入 `subsync-early.js`：浏览器本身能播 HEVC 时默认回答「不能」，让 streaming server 转码，和 Safari 走同一条路。想要不重新编码的原画，在字幕菜单上方的工具条里勾「HEVC 原画直通」，刷新页面后生效（只在能播 HEVC 的浏览器里显示）。转码最好走显卡：streaming server 设置里的硬件加速要开着，它的设置接口会把请求里没带的布尔项重置，网页设置页保存一次就可能把它关掉。
...
- 想让片库用 aria2 下载，装上发行版自带的 `aria2` 包即可（`aria2c` 在 PATH 里就会被用到，不需要配置，也不会装成服务）。
...
- **种子本身**：streaming server 的 `/<infoHash>/<fileIdx>/stats.json` 里有 `peers`、`unchoked`、`downloadSpeed`。同一个种子用 aria2 对照下载一分钟，能看出是种子慢还是下载器连不上 peer（我们因此让片库优先用 aria2）。
