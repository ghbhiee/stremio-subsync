...
- **下载到服务器（片库）**：播放时在字幕菜单上方的工具条点「下载到服务器」，或在影片详情页点顶栏的下载图标、在右侧面板里挑一个种子片源，服务器就在后台把整部片下完。**下载期间照常能看**：下载中的片子在片源列表里显示为「下载中 42% · 可边下边播」。下完之后，这部片的片源列表**第一条**就是「已下载 · 服务器本地」，从磁盘直接读，拖动秒开，做种的人都走了也能看；首页还多一个「已下载」目录。下载时会在片源自带的 tracker 之外补一批公共 tracker。任务状态落盘，服务重启后从断点继续；连接中断或长时间没数据会自动重试。
  - **两种下载器**：装了 `aria2c` 就用它下（先取种子信息、只选要的那个文件，`.aria2` 控制文件负责续传）；aria2 没装、连续三次失败或十分钟没有数据，就清掉它的目录、改由本机的 streaming server 下（把这个文件经它的 HTTP 接口从头读到尾，它会按顺序取齐所有分片并照常校验）。实测同一个「做种的人多但难连」的种子，75 秒里 aria2 连上 75–80 个 peer、速度爬到 3.7 MiB/s，streaming server 只连上 6 个、0.6 MB/s；好连的种子两者都能跑满链路。代价是用 aria2 时下载和观看不共用分片，边下边看时你看的那一段会由 streaming server 另下一份；用 streaming server 下载时则是共用的。`LIBRARY_DOWNLOADER=engine` 可以强制只用后者。面板里能看每个任务的进度、速度、连接数和剩余时间，看片库占用和磁盘剩余，取消下载或删除（删除要点两次确认）。片库有配额（默认 50 GB）并保证磁盘留有余量（默认 10 GB），超了就拒绝新任务。浏览器能直接播的文件（MP4 + H.264 + AAC）由网页服务器按 Range 直接发；其余（MKV、HEVC、AC3 等）仍经 streaming server 转封装，只是数据源换成了本地文件。从片库播放时字幕对齐读的也是本地文件。
