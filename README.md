...
- **AI 翻译（可选）**：把最准的英文字幕翻成简体中文，提供 `🤖 中英双语` 和 `🤖 中文` 两条字幕。默认用 DeepSeek `deepseek-flash`（关闭思考模式），按时间轴 20 分钟一段翻译，每段存盘，同一份字幕只花一次 token。
...
- 第一次看某个视频时，需要先下载前约 10 分钟才能对齐完；AI 翻译一集电视剧约 7 秒。播放器只加载一次字幕文件，若当时还没翻完，切到别的字幕再切回来即可刷新。
...
AI 翻译同一集 408 句：7.1 秒，6 次请求，输入 6.2k / 输出 4.4k tokens；再次播放直接读缓存，0 次请求。
